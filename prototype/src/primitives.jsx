...
function Sparkline({ data, width = 80, height = 28, color, fill = true, dashed = false }) {
...
function TeamChip({ team, size = "md" }) {
  const meta = window.TEAMS[team] || { color: "#2A4A37", name: team };
...
function PlayerCell({ player, mini = false }) {
  const team = window.TEAMS[player.team] || { color: "#2A4A37" };
  return (
...
// Big stat card
function StatCard({ label, value, suffix, delta, trend, foot, accent }) {
  return (
...
function Gauge({ value, max = 100, color = "var(--mint)", height = 4 }) {
...
Object.assign(window, { Sparkline, TrendDelta, TeamChip, PosPill, PlayerCell, MatchupBars, RiskDot, StatCard, SectionTitle, Gauge, TabBar });
