...
import { PLAYERS, CURRENT_WEEK, getGameLog } from "@/lib/mock"
import { PlayerCell, SectionTitle, TabBar } from "@/components/product/primitives"
import { IconCheck } from "@/components/product/icons"
...
export function PlayerTapeView({ onSelectPlayer }) {
  
  const [pickIds, setPickIds] = useState(["jallen"]); // multi-select up to 6
              ...
              return (
                <button key={p.id} onClick={() => toggleId(p.id)} style={{
                  ...
                  <div style={{ width: 18, height: 18, borderRadius: 4, border: `1.5px solid ${checked ? "var(--mint)" : "var(--green-600)"}`, background: checked ? "var(--mint)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    {checked && <IconCheck size={12} stroke="var(--black)" strokeWidth={3} />}
                  </div>
                </button>
              );
            })}
