...
import { TEAM_POSITION_WEEKS, TEAM_FUTURE_WEEKS, LEAGUE_POS_AVG, LEAGUE_STANDINGS } from "@/lib/mock"
...
export function MetricsView({ tab, setTab, onSelectPlayer }) {
  const tabs = [
...
function WeeklyPointsHeatmap() {
  ...
  const [scope, setScope] = useState("league"); // league / team / bench
...
function PositionMetricsView() {
  const [scope, setScope] = useState("team"); // team / league / bench
