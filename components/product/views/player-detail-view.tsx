...
import { PLAYERS, TEAMS } from "@/lib/mock"
import { Sparkline, TeamChip, PosPill, PlayerCell, RiskDot, SectionTitle, Gauge } from "@/components/product/primitives"
import { IconBolt, IconChevron, IconPlus, IconSwap } from "@/components/product/icons"

export function PlayerDetailView({ player, onBack }) {
  ...
  return (
    <div className="view-enter" style={{ maxWidth: 1280, margin: "0 auto", padding: "24px 32px 48px" }}>
      ...
      <button onClick={onBack} style={{
        ...
        <span style={{ transform: "rotate(180deg)", display: "inline-flex" }}><IconChevron size={14} /></span>
        Back to rankings
      </button>
      ...
      <div className="card" style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 24, position: "relative" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button className="btn ghost"><IconPlus size={14} /> Watchlist</button>
            <button className="btn outline"><IconSwap size={14} /> Trade</button>
            <button className="btn"><IconBolt size={14} stroke="var(--black)" /> Start</button>
          </div>
        </div>
