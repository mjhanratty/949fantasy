...
export function traj(seed, base, vol, trend = 0) {
  const out = [];
  let v = base;
  let rng = seed;
  for (let i = 0; i < 12; i++) {
...
// Roster (your team)
export const ROSTER_IDS = ["jallen", "cmcaff", "saquon", "jchase", "puka", "amon", "kelce", "henry", "smith"];
export const STARTER_IDS = ["jallen", "cmcaff", "saquon", "jchase", "puka", "amon", "kelce"]; // 7 starters + DST + K
export const BENCH_IDS = ["henry", "smith"]; // bench

// League scoring slots
export const LEAGUE_SLOTS = ["QB", "RB", "RB", "WR", "WR", "FLEX", "TE", "DST", "K"];

// Per-week per-position scores for your team — 10 played weeks
export const TEAM_POSITION_WEEKS = [
...
export const TEAM_FUTURE_WEEKS = [
...
// NFL cities for the Games map — lat/lng from stadium CSV, x/y via map projection
export const NFL_CITIES = Object.values(STADIUM_BY_TEAM).map((s) => ({
  team: s.team,
  city: s.label,
  indoor: s.indoor,
  lat: s.lat,
...
/** One map dot per coordinate; NYG/NYJ and LAC/LAR use home-team site for the week. */
export const NFL_MAP_MARKERS = buildMapMarkers(WEEK11_GAMES, WEEK11_HOME_AT_SHARED);
...
export function makeGameLog(player) {
...
const _logCache = {};
export function getGameLog(playerId) {
